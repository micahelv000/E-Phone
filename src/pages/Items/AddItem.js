...
function Row(props) {
  ...
  const handleSync = () => {
    devices.forEach((device) => {
      const initialDetails = initialItemDetails[device.slug] || {
        stock: 0,
        price: 0,
      };
      ...
      if (
        initialDetails.stock !== currentDetails.stock ||
        initialDetails.price !== currentDetails.price
      ) {
        axios
          .get(`https://phone-specs-api.vercel.app/${device.slug}`)
          .then((response) => {
            const data = response.data.data;
            const displaySpec = data.specifications.find(
              (spec) => spec.title === "Display"
            );
            let screenSize = null;
            if (displaySpec) {
              const sizeSpec = displaySpec.specs.find(
                (spec) => spec.key === "Size"
              );
              if (
                sizeSpec &&
                Array.isArray(sizeSpec.val) &&
                sizeSpec.val.length > 0
              ) {
